...
set.seed(s)
M1 = DLFM_fix(Ys, Xnew, RE = c("additive", "multiplicative"), gammapriors = rep(c(2, 1), 5), dist = "Exponential", avail = matrix(1, Time , N), R =2, burn = 1000, nscan = 5000, odens = 10, plot =FALSE, kappas = rep(0.001, 3))
set.seed(s)
M2 = DLFM_MH(Ys, Xnew, RE = c("additive", "multiplicative"), gammapriors = rep(c(2, 1), 5), dist = "Exponential", avail = matrix(1, Time , N), R =2, burn = 1000, nscan = 5000, odens = 10, plot =FALSE)
}
...
set.seed(s+2000)
	...
	U2 = sapply(1:N, function(i){rmvnorm(1, rep(0, Time), tau_u[2] * diag(Time))})
	U = lapply(1:Time , function(tp){cbind(U1[tp,], U2[tp,])})
	D = sapply(1:2, function(r){rmvnorm(1, rep(-1, Time), matrix(Exponential(dist_ij, kappas[3]), Time, Time, byrow = TRUE))})
	while (sum(c(D) > 0) > 0) {
	  D = sapply(1:2, function(r){rmvnorm(1, rep(-1, Time), matrix(Exponential(dist_ij, kappas[3]), Time, Time, byrow = TRUE))})
	}
	s2 = 1 / rgamma(1, 2, 1)
...
set.seed(s)
M3 = DLFM_Dunson(Ys, Xnew, RE = c("additive", "multiplicative"), gammapriors = rep(c(2, 1), 5), dist = "Exponential", avail = matrix(1, Time , N), R =2, burn = 1000, nscan = 5000, odens = 10, plot =FALSE, kappas = rep(0.001, 3))
